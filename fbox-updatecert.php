...
<?php
    ...
    $fboxurl    = (string) $xml->fboxurl;
    $fboxpasswd = (string) $xml->fboxpasswd;
    $fboxusername = (string) $xml->fboxusername;
    ...
    $xml = simplexml_load_file($fboxurl . "/login_sid.lua");
    $sid = (string) $xml->SID;
    $challenge = (string) $xml->Challenge;

    $response = $challenge . "-" . md5(mb_convert_encoding($challenge . '-' . $fboxpasswd, "UCS-2LE", "UTF-8"));
    $xml = simplexml_load_file($fboxurl . "/login_sid.lua?sid=" . $sid . "&username=" . $fboxusername . "&respons$    $sid = (string) $xml->SID;
    ...
    $response = file_get_contents($fboxurl . "/cgi-bin/firmwarecfg", false, $context);
    print_r($response);
    echo "\n";
?>
